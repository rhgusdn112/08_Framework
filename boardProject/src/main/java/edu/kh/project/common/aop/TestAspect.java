...
// @Aspect // 공통 관심사(advice)가 작성된 클래스임을 명시(AOP 동작에 참여)
@Slf4j
public class TestAspect {
	
	// Join Point :
	// 	advice(공통 관심사 코드)가 적용 될 수 있는 모든 지점(후보)
	
	// Pointcut : 
	//	후보 지점 중 실제 적용될 지점을 잘라낸 것
	//	(실제로 적용되고 수행할 지점)
	...
	public void controllerEnd(JoinPoint jp)	{
		// JoinPoint : AOP에서 사용 가능한 부가 기능을 가진 객체
		
		// AOP가 적용된 클래스명 얻어오기
		String className = jp.getTarget().getClass().getSimpleName();
		
		// jp.getSignature() : 실행된 타켓의 메서드 선언부
